...
function writeWholeDb(db, data) {
  ...
  const insertContact = db.prepare('INSERT INTO contacts (id, email, status, created_at, updated_at, data) VALUES (?, ?, ?, ?, ?, ?)')
  const insertNewsletter = db.prepare('INSERT INTO newsletters (id, email, status, created_at, updated_at, data) VALUES (?, ?, ?, ?, ?, ?)')
  const insertTestimonial = db.prepare('INSERT INTO testimonials (id, created_at, updated_at, data) VALUES (?, ?, ?, ?)')

  db.exec('BEGIN')

  try {
    db.exec('DELETE FROM users; DELETE FROM rooms; DELETE FROM spaces; DELETE FROM reservations; DELETE FROM payments; DELETE FROM contacts; DELETE FROM newsletters; DELETE FROM testimonials;')
...
export async function ensureDb() {
  ...
  try {
    ...
    const isEmpty = ['users', 'rooms', 'spaces', 'reservations', 'payments', 'contacts', 'newsletters', 'testimonials'].every((table) => tableCount(db, table) === 0)
    ...
    const current = {
      ...
      contacts: parseRows(db.prepare('SELECT data FROM contacts ORDER BY id DESC').all()),
      newsletters: parseRows(db.prepare('SELECT data FROM newsletters ORDER BY id DESC').all()),
